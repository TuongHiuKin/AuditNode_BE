...
public sealed class AdminUsersController(IIdentityAdminService identities, ILogger<AdminUsersController> logger) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<IdentityAdminUserDto>>> List([FromQuery] string? search, [FromQuery] int first = 0, [FromQuery] int max = 50, CancellationToken cancellationToken = default)
    {
        ...
        return Ok(await identities.ListUsersAsync(search, first, max, cancellationToken));
    }
