...
public sealed class WorkspaceShareOptionsService(AuditDbContext context, IIdentityAdminService identities) : IWorkspaceShareOptionsService
{
    ...
    public async Task<WorkspaceShareOptionsDto?> GetAsync(Guid workspaceId, string actorUserId, string? search, int first, int max, CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<ShareOptionUserDto> users = normalizedSearch.Length < 3
            ? Array.Empty<ShareOptionUserDto>()
            : (await identities.ListUsersAsync(normalizedSearch, 0, DirectoryCandidateLimit, cancellationToken))
                .Where(x => x.Enabled)
                .OrderBy(x => MatchRank(x, normalizedSearch))
                .ThenBy(x => x.Username, StringComparer.OrdinalIgnoreCase)
                .Skip(Math.Max(0, first))
                .Take(Math.Clamp(max, 1, 20))
                .Select(x => new ShareOptionUserDto(x.Id, x.Username, x.Email))
                .ToList();
    ...
    private static int MatchRank(IdentityAdminUserDto user, string search)
    {
        if (string.Equals(user.Username, search, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(user.Email, search, StringComparison.OrdinalIgnoreCase)) return 0;
        if (user.Username.StartsWith(search, StringComparison.OrdinalIgnoreCase) ||
            (user.Email?.StartsWith(search, StringComparison.OrdinalIgnoreCase) ?? false)) return 1;
        return 2;
    }
}
