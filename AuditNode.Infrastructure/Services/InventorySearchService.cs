    ...
    public async Task<IEnumerable<SearchResultDto>> SearchAsync(string keyword)
    {
        ...
        var appQuery = _context.Applications
            ...
            .Select(a => new SearchResultDto
            {
                ...
                Type = "APP",
                Title = a.AppName,
                Subtitle = $"On Server: {(a.Server != null ? a.Server.Hostname : "Unknown")} (Port: {(a.PortMappings.OrderBy(p => p.PortNumber).Select(p => p.PortNumber.ToString()).FirstOrDefault() ?? "N/A")})",
